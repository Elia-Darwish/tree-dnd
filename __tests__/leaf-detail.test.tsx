import '@testing-library/jest-dom'
import { act, render, screen, waitFor, within } from '@testing-library/react'
import { rest } from 'msw'
import { setupServer } from 'msw/node'

import { LeafDetails } from 'components/LeafDetails'

const id = 'imgly.A.1'

const server = setupServer(
  rest.get(`https://ubique.img.ly/frontend-tha/entries/${id}.json`, async (req, res, ctx) => {
    return res(
      ctx.json({
        id: 'e20d9958-abff-59f4-8561-0c76292dad73',
        createdAt: '2024-07-14T02:33:17.027Z',
        createdBy: 'ciftuzfaf@ovwatvuz.ug',
        lastModifiedAt: '2082-09-30T19:54:40.255Z',
        lastModifiedBy: 'fuh@wimumamok.lr',
        description:
          'Rebongig joeto gusku tiwwa vapumed cupi ni dep tijur uc rifrezlap peha dobpu gonnehe jija doduvce. Mis silot welug oho gitofpud cif eh ec awara voc volupif fi soimja. Ovi sovwombov farol bajifo vumutu zitoci eb hor dujot efinal fikrubebi wakimah toh rasovumi tipdolum ga wakorca ma. Azonev jajrali atu sufvegta ozudogin iplecvit futaki segjajem kowrize reroref lacevlu gen soufino hud defize filtimot.',
      }),
    )
  }),
)

beforeAll(() => server.listen())
afterAll(() => server.close())

describe('Leaf Detail', () => {
  it('fetches the data from the api', async () => {
    render(<LeafDetails />)

    const details = screen.getByRole('region', {
      name: /leaf details/i,
    })

    window.location.hash = `#${id}`
    await act(() => {
      window.dispatchEvent(new HashChangeEvent('hashchange'))
    })

    expect(details).toHaveAttribute('aria-busy', 'true')
    await waitFor(() => expect(details).toHaveAttribute('aria-busy', 'false'))

    within(details).getByRole('article', {
      name: new RegExp(`${id} details`),
    })
  })

  it('it renders an Error message when the data fetching fails', async () => {
    server.use(
      rest.get(`https://ubique.img.ly/frontend-tha/entries/${id}.json`, async (req, res, ctx) => {
        return res(ctx.status(500), ctx.json({ message: 'something went wrong' }))
      }),
    )

    render(<LeafDetails />)

    const details = screen.getByRole('region', {
      name: /leaf details/i,
    })

    await waitFor(() => expect(details).toHaveAttribute('aria-busy', 'false'))

    const error = within(details).getByRole('alert')

    expect(error).toBeInTheDocument()
  })
})
